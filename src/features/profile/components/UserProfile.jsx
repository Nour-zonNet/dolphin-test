  ...
  const handleAddSibling = async (siblingData) => {
    ...
    try {
      ...
      openStatusModal("ERROR", {
        ...
        message: err?.message || "حدث خطأ أثناء المحاولة. حاول مرة أخرى لاحقًا.",
      });
    }
  };
    ...
    <div className="flex items-center gap-4 md:gap-[37px] py-4 md:py-8">
      ...
      <div className="relative">
        <img
          ...
          alt="Profile"
          src={selectedImages[user.id] ? URL.createObjectURL(selectedImages[user.id]) : resolveAvatar(user?.profilePicture)}
          onError={handleImgError}
        />
        <label className="absolute bottom-0 lg:bottom-2.5 left-0 lg:left-2.5 cursor-pointer">
          <img className="w-6 md:w-8 h-6 md:h-8" alt="Edit" src="https://c.animaapp.com/mf29nm7vjLRxgE/img/frame-1.svg" />
          <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageChange(e, user.id)} />
        </label>
      </div>
          ...
          >
            ...
            {brothers.length === 0 && (
              <div className="flex flex-col items-center gap-2 py-6 text-gray-500">
                <img src="/empty.svg" alt="Empty" className="w-16 h-16 opacity-70" />
  ...
  />
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <p className="text-[12px] md:text-xl font-bold text-navyteal text-nowrap">{bro.student_name}</p>
                      {(bro.isPrimary || bro.canAddSiblings || bro.accountType === "primary") && (
                        <span className="text-xs bg-orange-100 text-orange-600 px-2 py-1 rounded-full font-bold">أساسي</span>
                      )}
                    </div>
                    <p className="text-[12px] md:text-base text-navyteal text-nowrap">{bro.class_name}</p>
                  </div>
                </div>

                {bro.id === user.id && (
                  <label className="cursor-pointer">
                    <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageChange(e, bro.id)} />
                    <img className="w-5 h-5 md:w-6 md:h-6" alt="Edit" src="https://c.animaapp.com/mf29nm7vjLRxgE/img/frame-1.svg" />
                  </label>
                )}
              </div>
            ))}
      ...
      <AddSiblingsModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleAddSibling} />
    </div>
  );
};
