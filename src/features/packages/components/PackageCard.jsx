...
const PackageCard = React.memo(({ item, color, image, status, daysRemaining }) => {
  ...
  const existingSchedule = useMemo(() => schedules?.[group_id], [schedules, group_id]);
  ...
  const handleOpenSchedule = useCallback(async () => {
    ...
    if (existingSchedule) {
      openWeeklyScheduleModal({
        data: { groupId: group_id, packageName: package_name, schedule: existingSchedule, image, color },
      });
      return;
    }
      ...
      <div className="relative w-full transition-transform duration-300 pr-0">
        ...
        >
          ...
          <div className="absolute flex items-start justify-end w-full -left-2 pt-8">
            <CardKite fill={color} className="relative -left-5 w-[120px] sm:w-[180px] md:w-[180px]" />
          </div>

          {/* Header */}
          <div className="flex flex-row xs:items-center gap-2 relative z-10 text-white px-3 py-4 bg-gradient-to-r">
            {image && (
              <div style={{ backgroundColor: color }} className="w-8 h-8 sm:w-12 sm:h-12 rounded flex items-center justify-center p-1">
